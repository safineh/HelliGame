...
def playerCollision(item):
	if item.tag == "bomb":
		...
		if health.text <= 0:
			health.text = 0
			game.pause=True
			game.Label("GAME OVER", (220, 240), color=game.WATER, size=64, tag="temp")
			r = game.Label(" Restart ", (270, 340), color=game.WHITE, back=game.WATER, size=64, radius=0.4, tag="temp")
			r.click = Restart
